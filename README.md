...
2. **英文感知**：自动检测最近 5 条消息中是否有英文内容（含代码块、工具返回结果），检测到立即注入
3. **周期刷新**：每 3 条用户消息刷新一次，防止指令被长对话稀释
4. **智能去重**：先剥离旧指令再注入新指令，避免重复堆叠

### 英文内容检测

插件会分析上下文中所有消息的英文含量（ASCII 字母占比 > 50%），覆盖：
...
1. 在 [Releases](https://github.com/hayou2002/think-lang-control/releases) 下载 `思考语言控制-v2.1.0.zip`
...
```bash
zip -r 思考语言控制-v2.1.0.zip manifest.json extensions/ -x "*.DS_Store"
```
...
- `isEnglishHeavy` 错误地剥离了代码块和行内代码，但代码本身就是英文内容（变量名、关键字、注释），是语言引力的主要来源

**增强：**

- 指令文本强化：明确提及 GitHub、Stack Overflow、PR、Issue、README 等场景，增加"用户无法理解"的后果说明
- 检查窗口从最近 3 条消息扩大到 5 条
- 最短检测长度从 30 降到 10 字符，小段代码也能触发检测
- 指令注入位置从第一条用户消息改为最后一条，最大化推理时的影响力
